/**
 * @name UserNotesLoader
 * @version 1.0.0
 * @description Lightweight BetterDiscord launcher/updater for UserNotes. Downloads the latest payload each time it starts.
 * @author Sleek
 * @source https://github.com/s4dic/discord/tree/main/BetterDiscord%20Plugins/UserNotes/UserNotesLoader.plugin.js
 */

module.exports = class UserNotesLoader {
    constructor() {
        this.remoteUrl = "https://raw.githubusercontent.com/s4dic/discord/refs/heads/main/BetterDiscord%20Plugins/UserNotes/data/UserNotes.js";
        this.remote = null;
        this.enabled = false;
        this.generation = 0;
        this.status = "Idle";
    ...
    async reloadLatest(showToast = true) {
        if (!this.enabled) {
            BdApi.UI.showToast("UserNotes: enable the launcher before reloading", {type: "info"});
            return;
        }
    ...
    async loadLatest(token, showToast) {
        ...
        try {
            ...
            if (showToast) {
                BdApi.UI.showToast(
                    this.remoteVersion
                        ? "UserNotes remote v" + this.remoteVersion + " loaded"
                        : "UserNotes latest remote payload loaded",
                    {type: "success"}
                );
            }
        } catch (error) {
            ...
            console.error("[UserNotesLoader] Failed to load remote payload:", error);
            BdApi.UI.showToast("UserNotes: remote update/load failed", {type: "error"});
        }
    }
    ...
    stopRemote() {
        ...
        try {
            ...
            console.error("[UserNotesLoader] Failed to stop remote plugin:", error);
        }
        this.remote = null;
        this.remoteVersion = null;
    }
    ...
    getSettingsPanel() {
        ...
        title.textContent = "UserNotes Launcher";
        ...
        try {
            ...
            console.error("[UserNotesLoader] Failed to build remote settings panel:", error);
        }
